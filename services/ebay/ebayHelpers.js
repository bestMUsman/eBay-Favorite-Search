...
function fetchItemsFromEbay(req, res, next) {
  let URL = `http://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsByKeywords&SERVICE-VERSION=1.0.0&SECURITY-APPNAME=${API_KEY}&GLOBAL-ID=EBAY-US&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&keywords=${req.body.search}&itemFilter(0).name=ListingType&itemFilter(0).value=${req.body.listingType}&paginationInput.entriesPerPage=${req.body.maxResults}&sortOrder=${req.body.sortBy}`;
